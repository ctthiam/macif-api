...
async function main() {
    ...
    const catId = (name) => cats.find((c) => c.name === name)?.id ?? cats[7].id;
    const productsData = [
    ...
    const products = await Promise.all(productsData.map((p) => prisma.product.create({
        data: {
            name: p.name,
            unit: p.unit,
            buyPrice: p.buy,
    ...
    const customersData = [
        { name: "Mamadou Diop", phone: "776543210", type: "individual", address: "HLM Grand Yoff, Dakar" },
        { name: "Fatou Sall", phone: "775432109", type: "individual", address: "Médina, Dakar" },
        { name: "Ibrahima Guèye", phone: "774321098", type: "individual", address: "Parcelles Assainies, Dakar" },
        { name: "Moussa Ba", phone: "773210987", type: "individual", address: "Pikine Est" },
        { name: "Aminata Fall", phone: "771987654", type: "individual", address: "Guédiawaye" },
        { name: "Cheikh Mbaye", phone: "779876543", type: "individual", address: "Cambérène, Dakar" },
        { name: "Oumar Ndiaye", phone: "778654321", type: "individual", address: "Thiaroye" },
        { name: "Entreprise BTP Sérigne & Fils", phone: "772109876", type: "btp", address: "Zone Industrielle Dakar" },
        { name: "SARL Construction Sénégal", phone: "778765432", type: "professional", address: "Plateau, Dakar" },
        { name: "Aissatou Diallo", phone: "770123456", type: "individual", address: "Sicap Liberté 5" },
    ];
    const customers = await Promise.all(customersData.map((c) => prisma.customer.create({ data: { ...c, shopId: SHOP_ID } })));
    ...
    const expensesData = [
        { cat: "transport", amount: 15000, desc: "Livraison ciment chez client BTP", db: 29 },
        { cat: "electricity", amount: 28500, desc: "Facture SENELEC du mois", db: 25 },
        { cat: "other", amount: 5000, desc: "Papier d'emballage et sacs plastique", db: 23 },
        { cat: "salary", amount: 75000, desc: "Salaire gardien Aliou — Avril", db: 20 },
        { cat: "transport", amount: 8000, desc: "Transport marchandises SOMETA", db: 18 },
        { cat: "other", amount: 12000, desc: "Réparation étagère de stockage", db: 15 },
        { cat: "other", amount: 3500, desc: "Fournitures bureau (stylos, cahiers)", db: 12 },
        { cat: "water", amount: 9200, desc: "Facture SDE du mois", db: 10 },
        { cat: "salary", amount: 45000, desc: "Salaire vendeur — Mi-mai", db: 8 },
        { cat: "transport", amount: 6000, desc: "Carburant livraison Parcelles", db: 6 },
        { cat: "other", amount: 4500, desc: "Déjeuner équipe vendredi", db: 4 },
        { cat: "rent", amount: 150000, desc: "Loyer boutique — Mai 2026", db: 2 },
        { cat: "other", amount: 2500, desc: "Nettoyage boutique", db: 1 },
        { cat: "transport", amount: 7500, desc: "Livraison carrelage client Cambérène", db: 0 },
    ];
